 ...
 * ImageControl widget.
 */
define('icybee/images/image-control', [

	'icybee/images/save-operation'
	...
	return new Class({
		...
		initialize: function(el, options) {
			...
			this.element = el
			this.fileElement = el.querySelector('[type="file"]')
			this.nidElement = el.querySelector('[type="hidden"]')
			this.progressPositionElement = el.querySelector('.progress-position')
			this.alertContentElement = el.querySelector('.alert .content')
		 ...
		 *
		 * @param {number} position The position expressed as a float from 0 to 1.
		 */
		setPosition: function(position) {

			this.progressPositionElement.setStyle('width', position * 100 + '%')
			this.element.querySelector('.progress-label').innerHTML = Math.round(100 * position) + '%'
...
!function (Brickrouge) {
	...
	Brickrouge.register('ImageControl', function (element, options) {
		...
		if (!Constructor)
		{
			Constructor = require('icybee/images/image-control')
		}
